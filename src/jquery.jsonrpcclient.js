...
// (function ($) {
    ...
    JsonRpcClient._batchObject.prototype._execute = function () {
        ...
        if (socket !== null) {
            ...
            for (var k = 0; k < this._requests.length; k++) {
                var calls = this._requests[k];
                batchRequest.push(calls.request);

                // If the request has an id, it should handle returns (otherwise it's a notify).
                if ('id' in calls.request) {
                    handlers[calls.request.id] = {
                        successCb: calls.successCb,
                        errorCb: calls.errorCb
                    };
                }
            }
