<template>
    <div class="abnormal-box">
        <CheckUrlBind
            v-if="showFind && bindType == 'url'"
            @checkAccount="search"
        ></CheckUrlBind>
        <CheckActBind
            v-if="showFind && bindType == 'act'"
            @checkAccount="search"
        ></CheckActBind>
        ...
        <div class="animated fadeIn" v-show="showResult">
            <div class="ab-result-box ab-result-box1" v-if="abType == 'ab1'">
                <div class="result-item">
                    执行任务的账号“ <span class="blue-t">{{ con_name }}</span> ”
                    已被其它人关联使用，需要您到{{ curPlat.name }}切换到“
            ...
            <div class="ab-result-box ab-result-box2" v-if="abType == 'ab3'">
                ...
                <div class="result-item" style="text-align: left">
                    您已用“ <span class="blue-t">{{ con_name }}</span> ”
                    账号执行了{{ errDataCount }}条任务，很可能在{{
                        curPlat.name
                    }}执行任务时没有切换到“
                    <span class="blue-t">{{ curPlat.user.name }}</span> ” 账号。
                    您是否更改关联账号为“
                    <span class="blue-t">{{ con_name }}</span> ”
                    ？更改后以上已执行任务将完成验收。
                </div>
                <div class="ab-r-hd-btn-box">
                    <Button
                        class="ab-r-hd-btn primary-line-btn"
                        size="large"
                        @click="back"
                        type="default"
                        >不更改</Button
                ...
                <div class="task-content">
                    <div class="task-list-top dpflex">
                        “{{ con_name }}”已执行任务
                    </div>
...
import CheckUrlBind from "../components/CheckUrlBind";
import CheckActBind from "../components/CheckActBind";
...
export default {
    ...
    methods: {
            ...
            API.checkChangeConfirm(params)
                .then(function(res) {
                    ...
                    if (_d.result) {
                        ...
                        userInfo.bind_account[currPlat.app_name] = currPlat;
        ...
        search(value) {
            var _ = this;
            window.console.log("开始智能排查: ", value);
            if (value != "") {
                _.showFind = false;
                _.loading = true;
                var params = new URLSearchParams();
                ...
                _.oauth_user_info = JSON.stringify(value);
                params.append("oauth_user_info", _.oauth_user_info);
                params.append("platform", platform);
                params.append("account_id", account_id);
                API.accountCheck(params)
                    .then(function(res) {
                        if (res.data.result) {
                            var _d = res.data;
                            _.is_check = _d.check;
                ...
                .then(function(res) {
                    ...
                    if (_d.result) {
                        if (_d.progress == "100.00%") {
                            ...
                            if (_d.data_result && _d.data_result.length > 0) {
                                // 有任务
                                _.showTips = false;
                                ...
                                for (let i = 0; i < iss.length; i++) {
                                    ...
                                    if (dsc.article_type) {
                                        // 问题 < 精华 < 精选
                                        if (
                                            dsc.article_type.indexOf("qa") > -1
                                        ) {
                                            tagName = "问题";
                                            tagColor = "bg-blue";
                                        }
                                        if (
                                            dsc.article_type.indexOf("jh") > -1
                                        ) {
                                            tagName = "精华";
                                            tagColor = "bg-orange";
                                        }
                                        if (
                                            dsc.article_type.indexOf("jx") > -1
                                        ) {
                                            tagName = "精选";
                                            tagColor = "bg-blue";
                                        }
                                        if (
                                            dsc.article_type.indexOf("video") >
                                            -1
                                        ) {
                                            hasVideo = true;
                                        } else if (
                                            dsc.article_type.indexOf("img") > -1
                                        ) {
                                            hasImg = true;
                                        }
                                    }
                                    ...
                                    var tb_item = {
                                        title: {
                                            ...
                                            time: UTILS.getDateDiff(
                                                dti.task_create_at * 1000
                                            ),
                                            error_status:
                                                dti.custom_fields_obj
                                                    .error_status || null,
                                            operate_account:
                                                dti.custom_fields_obj
                                                    .operate_account || null
                                        }
                                    };
                                    _dt.push(tb_item);
                                    var cf_tk = {
                        ...
                        for (let i = 0; i < iss.length; i++) {
                            ...
                            if (dsc.article_type) {
                                if (dsc.article_type.indexOf("jh") > -1) {
                                    ...
                                    dsc.article_type.indexOf("video") > -1
                                ) {
                                    hasVideo = true;
                                } else if (
                                    dsc.article_type.indexOf("img") > -1
                            ...
                            var tb_item = {
                                title: {
                                    ...
                                    error_status:
                                        dti.custom_fields_obj.error_status ||
                                        null,
                                    operate_account:
                                        dti.custom_fields_obj.operate_account ||
                                        null
                                }
                            };
                            _dt.push(tb_item);
    ...
    created() {
        ...
        this.bindType = this.bindTypeSort[this.curPlat.platform];
    }
};
</script>
